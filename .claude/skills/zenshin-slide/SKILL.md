...
- **コールアウトを意味に応じて色分けして使う**:
  - `>`（blockquote）= takeaway（そのページで発表者が言い切る、ラベル・背景なしの中央1文）
...
- takeawayは**中央揃えの1文・1行**に収める（目安は全角35文字前後）。`POINT`や`結論`などのラベル、背景、枠は付けない。文全体を太字にせず、核心語だけ`***太字***`でゴールド下線を引く
...
- 2〜3カラムは`<div class="columns">`（3列は`columns three`）、対比カードは`<div class="card">`、同一テーマの項目リストは`<div class="key-list">` + `<div class="key-item">`、KPIタイルは`<div class="stats">` + `<div class="stat">`
- **カードは独立した2〜3グループに限定する**: A/B対比や、境界を明確に分ける必要がある大きな概念だけに使う。同格の確認項目を要素ごとにカード化すると枠が増えて情報の流れを分断するため、3項目以上は`key-list`か通常の単一列リストにする
...
- **独立した箇条書きを裸の2カラムに分けない**: 読む順番が左右に散り、項目のまとまりも弱くなる。A/Bの明確な対比なら2枚のカード、単一テーマの3〜5要素なら`key-list`、順番に読む内容なら通常の単一列リストにする
...
- 4:3のYes / Noフローは原則`flowchart LR`、質問5個程度まで、1ノード2行以内にする。到達点は質問ノードと形を分け、標準テーマの可読性を壊す独自配色を増やさない
