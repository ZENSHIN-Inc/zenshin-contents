...
[ZENSHINの技術ブログ](https://contents.zenshin-inc.co.jp/blog/) に、記事ごとの OGP 画像を自動生成する仕組みを入れました。Slack や X で記事 URL を共有したときに、記事固有のサムネイルが展開されます。

ちなみに、下のリンクカードに表示されているのがこの記事の OGP です（リンクカードも後述の仕組みと同様に、ビルド時に `<meta og:image>` を取得して描画しています）。

[https://contents.zenshin-inc.co.jp/blog/astro-og-image-generation/](https://contents.zenshin-inc.co.jp/blog/astro-og-image-generation/)
