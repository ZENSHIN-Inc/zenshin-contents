---
marp: true
theme: zenshin
lang: ja
paginate: true
title: 生成 AI ツール戦略 2026年7月版 — Claude Code と Codex を押さえれば十分な理由
description: モデルの勢力図（トップ 2 社 + 厚い追走ティア）から、なぜ Claude Code と Codex の 2 つを使い込み先にするのか、コスパ・タイパの見方、課金戦略、特化ツールとの付き合い方までを整理します。
footer: © ZENSHIN Inc.
---

<!-- _class: lead -->
<!-- _paginate: false -->

# 生成 AI ツール戦略

## 2026年7月版<br>**Claude Code と Codex** を押さえれば十分な理由

![w:240](../assets/brand/ZENSHIN-logo-white.webp)

2026年7月6日
株式会社ZENSHIN CTO 高橋俊

---

<!-- _class: center -->

# 結論から

***2026年7月現在、使い込み先は次の 2 つです。***

| 会社 | モデル系列 | CLI ツール |
| --- | --- | --- |
| **Anthropic** | Claude 系 | **Claude Code** |
| **OpenAI** | GPT 系 | **Codex** |

- この 2 つを使い込めば、世の中のほとんどの AI ツールは**代替できる**
- ツール連携・長時間実行・業務定型化は、この 2 社が先行している
- **GPT-5.6 Sol** は Opus 4.8 を明確に上回り、Fable 5 に 1 点差。**2 強の構図は維持**

このスライドでは「**なぜそう言い切れるのか**」を順に説明します。

---

<!-- _class: divider -->
<!-- header: "" -->

# 業界地図 — 2 強と厚い追走

1. **業界地図 — 2 強と厚い追走**
2. 世の中の AI ツールの正体
3. なぜこの 2 社が突出するのか
4. 課金戦略と副次効果

---

<!-- header: 1. 業界地図 — 2 強と厚い追走 -->

# モデルの勢力図 — 3 ティア構造

| ティア | モデル系列（代表モデル） |
| --- | --- |
| **T1 — 使い込み先** | **Claude 系**（Fable / Opus / Sonnet）・**GPT-5.6 系**（Sol / Terra / Luna） |
| **T2 — 有力追走** | **Grok**（4.5）・**GLM**（5.2）・**Gemini**（3.5 Flash / 3.1 Pro） |
| **T3 — コスパ勢** | **Qwen**・**DeepSeek**・**Kimi**・**MiniMax** など |

出典: [Artificial Analysis](https://artificialanalysis.ai/)（2026年7月10日時点。代表モデルの位置づけ）

- ***使い込み先は T1 の 2 系列***。ツール・課金・ノウハウの積み上げ先として強い
- **T2 は無視できない追走**。Grok / GLM / Gemini が実務候補に入る
- T3 は**コスパやタイパを武器に**しのぎを削る。乗り換え先の候補として見る

---

<!-- _class: center -->

# GPT-5.6 Sol の位置づけ

| 観点 | 評価 |
| --- | --- |
| **知能** | AA 指数 **59**。Fable 5（60）に 1 点差、Opus 4.8（56）を上回る |
| **コスト** | 1 タスク **$1.04**。Fable 5（$2.75）の約 3 分の 1 |
| **速度** | **83 tokens/s**。Fable 5（63）より高速 |
| **コーディング** | Codex で Coding Agent Index **80**。現行トップ |

> Sol は Opus 4.8 を明確に上回った。一方、Fable 5 は総合知能と長時間の知識業務で首位を維持。***OpenAI / Anthropic の 2 強という判断は変わらない***。

---

<!-- _class: center -->

# 知能だけでなくコスパでも見る

![w:530](../assets/slides/2026-07-06-ai-tool-strategy/aa-intelligence-vs-cost.png)

左上ほど優秀。**GPT-5.6 Sol は Fable 5 に 1 点差で、コストは約 3 分の 1**。
Opus 4.8 を上回り、GPT 系の優位が鮮明になった。

---

<!-- _class: center -->

# タイパ（速度）でも見る

![w:530](../assets/slides/2026-07-06-ai-tool-strategy/aa-intelligence-vs-time.png)

**GPT-5.6 Sol は 83 tokens/s**。Fable 5（63）より速く、Opus 4.8 より高性能。
速度まで含めると、Sol の優位はさらに大きい。

---

# 我々の業務の本質は「文書作成」

| 業務 | 正体 |
| --- | --- |
| メール・企画書・報告書 | **文書作成** |
| コード | **文書作成**（プログラムもテキスト） |
| スライド生成 | **文書作成**（構成と文章が本体） |
| グラフ・可視化 | 文書を **コードで可視化** したもの |

> 各社の競争は ***「どのモデルが文書とコードの生成に最も向いているか」*** に集約される。日々の使い込みは **この 2 社** に集中する。

---

<!-- _class: divider -->
<!-- header: "" -->

# 世の中の AI ツールの正体

1. 業界地図 — 2 強と厚い追走
2. **世の中の AI ツールの正体**
3. なぜこの 2 社が突出するのか
4. 課金戦略と副次効果

---

<!-- header: 2. 世の中の AI ツールの正体 -->

# ほとんどの AI ツールは「ラッパー」

Genspark、Manus などのツールの中身は、**Claude や GPT に適した指示文を被せているだけ**のケースがほとんど。

- モデルが賢くなるほど、ラッパー側の工夫の価値は**目減りする**
- 実際、この 1 年でラッパー系ツールの優位性はほぼ消えた

> **業務特化ツールの判断基準**: 中身がラッパーなら ***スキルを設定した Claude Code / Codex でほぼ同じことができる***。買う前に「生の 2 社 + スキル設定」で代替できないかを試す。

---

# 有名ツールの裏側も主要モデル

| ツール | 裏側のモデル |
| --- | --- |
| **Microsoft 365 Copilot** | GPT-5 系 + Claude（両対応） |
| **GitHub Copilot / Cursor** | GPT / Claude / Gemini を切替 |
| **Genspark** | GPT / Claude / Gemini を並列実行 |
| **Manus** | Claude + Qwen |
| **OpenClaw**（旧 Clawdbot） | Claude ほか好きなモデルを接続 |

「独自 AI」に見えるツールも、中身は ***主要モデル + 指示文と UI*** がほとんど。

---

# 特化ツールとの付き合い方 — V0 の例

**要所で手軽な特化ツールが有効な場面はある。**

- 例: **V0**（ノーコードでシステムのプロトタイプを作るツール）
- 月 $20 程度なら、特定用途に課金する価値があるケースも

<div class="callout warn">

特化ツールは**すぐ新しいツールに追い抜かれ**、そこで培った使い方のノウハウが**陳腐化しやすい**。リスクを織り込んで課金額を決める。

</div>

<div class="callout good">

Anthropic と OpenAI は ***1 年間トップランナーで走り抜いた実績***がある。学習投資を積み上げる先としては、この 2 社が最も安全。

</div>

---

<!-- _class: divider -->
<!-- header: "" -->

# なぜこの 2 社が突出するのか

1. 業界地図 — 2 強と厚い追走
2. 世の中の AI ツールの正体
3. **なぜこの 2 社が突出するのか**
4. 課金戦略と副次効果

---

<!-- header: 3. なぜこの 2 社が突出するのか -->

# 差がつくのは「ツールの使いこなし」

モデル単体の賢さ以上に、***外部ツールをどれだけうまく使えるか***で差がつく時代。

| 仕組み | できること |
| --- | --- |
| **スキル** | 業務の手順・知識を読み込ませて専用処理をさせる |
| **MCP・コネクター** | Gmail・カレンダー・Excel などと直接つながる |
| **プラグイン** | 用途ごとの機能拡張 |

Claude と OpenAI はこの「**ツールを使わせる仕組み**」の作り込みが頭一つ抜けており、追走勢との差はまだ大きい。

---

# 「Google 製品なら Gemini」は誤解

「Office と連携するなら Copilot」も同じ誤解です。

- Gmail・カレンダー・スプレッドシート・Excel などとの連携は、
  **MCP という標準規格**で公開されている
- 標準規格なので、**Claude でも GPT でも GLM でも同じ経路**でつながる
- 接続の設定も、**コネクターやスキルを有効化するだけ**で難しくない

> どのモデルからも同じ場所につながる以上、***残るのはモデルの性能差だけ***。エコシステムの囲い込みを理由にモデルを選ぶ必要はない。

---

# 「CLI 版」とは — 黒い画面ではない

**CLI** = コマンドラインインターフェース。ターミナルで動くツール形式のこと。

- Claude Code / Codex は元々**黒い画面のエンジニア向けツール**として誕生
- いまは CLI をラップした**デスクトップアプリ**からフル機能を使える
- UI/UX も洗練されており、***非エンジニアでも使いこなせる***

<div class="callout info">

本資料の「CLI 版」は「**ローカル PC で動く版**」の総称。ブラウザで開く Web 版（チャット画面）との対比で使っている。

</div>

---

# CLI 版こそが本命 — Claude Code と Codex

**CLI 版（Claude Code / Codex）にできること**

- ローカル PC のファイルに直接アクセスして作業する
- **PC 上のブラウザを操作**する
- **長時間ループで動き続ける**タスクをこなす
- **スキル**（定型手順をまとめたプロンプトファイル）で業務を定型化する
- ローカルのスクリプトや MCP と組み合わせ、**業務専用の処理**を組む

<div class="callout warn">

Web ブラウザ版とは別物。***Web 版だけでは実力の半分も出ない。CLI 版の使いこなしが 2026 年の分岐点。***

</div>

---

# 「Web 版とは別物」の中身

| | Web 版（ブラウザ） | CLI 版（ローカル） |
| --- | --- | --- |
| ファイル | 添付した分だけ | **PC 内全部を直接編集** |
| 操作 | チャット回答まで | **アプリも自分の Chrome も操作** |
| 実行時間 | 1 問 1 答が基本 | **数時間の放置実行** |
| 積み上げ | 会話ごとにリセット | **スキル・設定が資産化** |

<div class="callout info">

「ローカル LLM」ではなく、***モデルはクラウド最上位のまま、手足だけがローカルに来る***。ブラウザは**ログイン認証ごと操作できる**。

</div>

---

# 使い込みは 2 社、観測は T2 まで

Claude と OpenAI は**ツール活用のリードカンパニー**。

- 日々使い込む先は、**Claude Code / Codex** に絞る
- ニュースとして追う範囲は、***Grok / GLM / Gemini*** まで広げる
- ただし学習投資は、CLI・スキル・MCP が成熟した 2 社に寄せる

<div class="callout info">

「使う対象」と「観測する対象」を分ける。全部を使い込もうとすると、ノウハウが分散して積み上がらない。

</div>

---

<!-- _class: divider -->
<!-- header: "" -->

# 課金戦略と副次効果

1. 業界地図 — 2 強と厚い追走
2. 世の中の AI ツールの正体
3. なぜこの 2 社が突出するのか
4. **課金戦略と副次効果**

---

<!-- header: 4. 課金戦略と副次効果 -->
<!-- _class: center -->

# 推奨する課金プラン

| 使い方 | 目安 |
| --- | --- |
| まず試す・ライトな通常業務 | $20 |
| コーディングや大量の資料作成が入る | $100 |
| コーディング多用のエンジニア | **$200** |

- おすすめは **2 社とも $200**。様子見なら **$200 + $20 併用**から
- 月 $200 ≒ **約 3 万円で GPT-5.6 系 / Claude 最上位級が使えるのは破格**
- ***重要なのは、サブスク定額で最上位級が使えること***（従量課金ではない）

---

# 将来の値上げに備える

- 超トップモデル（**GPT-5.6 / Fable 5 級**）は、今後**従量課金・高額化**の可能性
- 一歩後ろを **Grok 4.5・GLM-5.2・Gemini・Qwen** などの
  **追走モデル**が追う — **約 1 年で今のトップ水準に到達**する
- いま 2 社で貯めるノウハウは、
  ***スキル化 + 省コンテキスト・省トークンの業務設計***

> このノウハウは**追走モデルにそのまま移植できる**。乗り換える頃には T2 / T3 も**実務ラインを超えてくる**ため、「追走モデル + スキル」で業務を回す**保険になる**。

---

# この戦略が変わるタイミング

- 転機は、**2 社が定額サブスクをやめて主力モデルまで従量課金化**したとき
- そのときの選択は 2 つ — **従量課金を受け入れる**か、
  **追走モデルへ乗り換える**か
- どのモデルも**スキル・MCP・コネクターと同じ概念**で拡張されており、
  ***積んだノウハウは乗り換え先でもそのまま効く***

> 2 社の進化は「よりオープン・外部接続・自律・長時間」の方向で、**特定モデルへのロックインは起きにくい**。それまでは 2 社で学び切り、その時が来たら **Grok / GLM / Gemini などへ乗り換え**ればいい。

---

# 副次効果 — AI ニュースの解像度が上がる

2 社のツールを使い込むほど、業界の見え方が変わります。

- 新しい AI ツールのニュースを見たとき、
  「これは Claude の定期実行機能を使っているな」
  「これは既存機能のラッパーだな」と**裏側が推測できる**
- 「すごいものが突然出た！」という話題にも**慌てずに済む**
- 一段高い視点から、俯瞰して AI ニュースを評価できるようになる

<div class="callout good">

ツールの習熟が、そのまま***情報の目利き力***になる。

</div>

---

<!-- header: "" -->
<!-- _class: center -->

# まとめ

1. モデルは **2 強 + 厚い追走**。使い込み先は **Claude 系 / GPT 系**
2. 世の AI ツールの大半は**そのラッパー**。業務の本質は**文書作成**
3. 差がつくのは**ツールの使いこなし**。スキル・MCP・ブラウザ操作・
   長時間実行まで扱える **CLI 版（Claude Code / Codex）が本命**
4. 使い込みはこの 2 社。情報収集は **T2（Grok / GLM / Gemini）まで観測**
5. 課金は用途別に **$20 / $100 / $200** — 推奨は **2 社とも $200**。
   **約 3 万円の定額で最上位級**が使えるのは破格
6. **定額が崩れたら追走モデルへ乗り換え**。それまで 2 社で学び切る

2026年7月現在、
***Claude Code と Codex を腰を据えて使い込むのが最善手***です。

---

<!-- _class: lead -->
<!-- _paginate: false -->

# ありがとうございました

![w:240](../assets/brand/ZENSHIN-logo-white.webp)

AI ツール選定・導入のご相談を承っています。
「うちの業務ならどう組めばいい？」から、お気軽にどうぞ。

**お問い合わせ**: [www.zenshin-inc.co.jp/contact](https://www.zenshin-inc.co.jp/contact)
