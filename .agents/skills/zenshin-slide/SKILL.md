...
- **コールアウトを意味に応じて色分けして使う**:
  - `>`（blockquote）= 結論（比較・根拠・説明から導かれる判断や示唆を1文で置く）
...
- **「結論」はページを締める装飾ではない**: 本文から導ける判断があり、読者が次に何を選ぶ・理解するかを明確にできる場合だけ使う。タイトルや本文の繰り返し、注記、出典の読み方には使わず、INFOやCAUTIONへ振り分ける
- 2〜3 カラムは `<div class="columns">`（3列は`columns three`）、対比カードは `<div class="card">`、単一テーマの要素カード群は`<div class="cards">`、KPIタイルは`<div class="stats">` + `<div class="stat">`
- **カードに装飾的な上線・横線を付けない**。薄い枠と余白でグループを示し、色は見出しや意味のあるコールアウトに使う
...
- **独立した箇条書きを裸の2カラムに分けない**: 読む順番が左右に散り、項目のまとまりも弱くなる。A/Bの明確な対比なら2枚のカード、単一テーマの3〜5要素なら`cards`、順番に読む内容なら単一列のリストにする
