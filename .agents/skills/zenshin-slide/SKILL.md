...
- 推奨ページ構成: lead 表紙（`<!-- _paginate: false -->`）→「結論から」（center）→ 章扉（lead）と本文の繰り返し → まとめ（center）→ lead 謝辞
- 表紙・謝辞のロゴ: `![w:240](../assets/brand/ZENSHIN-logo-white.webp)`（lead の濃紺背景用に白ロゴ）

## zenshin テーマの部品と書き方ルール

テーマは `themes/zenshin.css`（v2.2）。部品の一覧と使い方はテーマ CSS 冒頭のコメントが正。要点:
...
- **強調は `**太字**`** — テーマがマーカーハイライトを敷く（ブランドの署名的表現）。1 ページに数カ所まで
...
本文領域は約 **832x592px、1 行あたり全角約 33 文字**。後から削るのではなく、書く時点で次に収める:
